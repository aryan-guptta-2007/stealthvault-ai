...
async def register_tenant(
    ...
    tenant_check = await db.execute(select(DBTenant).where(DBTenant.name == payload.tenant_id))
    if tenant_check.scalars().first():
        raise HTTPException(status_code=400, detail="Organization ID already taken")
    ...
    await log_audit(
        ...
        tenant_id=new_tenant.id,
        result="SUCCESS",
        message=f"New SaaS onboarded: {payload.tenant_id} ({payload.plan})",
